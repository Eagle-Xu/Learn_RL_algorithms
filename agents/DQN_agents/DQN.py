...
class DQN(Base_Agent):
    ...
    def step(self):
        ...
        while not self.done:
            ...
            if self.time_for_q_network_to_learn(): #当达到足够的存储经验后，每三步学习一次
                for _ in range(self.hyperparameters["learning_iterations"]): #每次迭代的次数
                    self.learn()
            self.save_experience() #保存一个transition
            self.state = self.next_state # this is to set the state for the next iteration
            self.global_step_number += 1 #每存储一个transition加1
    ...
    def learn(self, experiences=None):
        ...
        actions_list = [action_X.item() for action_X in actions ]  # 存储本次的所有行动

        self.logger.info("Action counts {}".format(Counter(actions_list))) #统计动作的出现的种类数
