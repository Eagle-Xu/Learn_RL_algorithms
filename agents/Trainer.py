...
class Trainer(object):
    ...
    def run_games_for_agent(self, agent_number, agent_class):
        ...
        agent_results = [] # 存储该智能体的结果
        agent_name = agent_class.agent_name # 获取当前智能体名字
        agent_group = self.agent_to_agent_group[agent_name] # 获取当前智能体所在的组
        agent_round = 1
        for run in range(self.config.runs_per_agent):
            agent_config = copy.deepcopy(self.config) # 复制当前智能体的配置
            ...
            if self.config.randomise_random_seed: agent_config.seed = random.randint(0, 2**32 - 2) # 生成随机种子
            agent_config.hyperparameters = agent_config.hyperparameters[agent_group] # 传入智能体所在组的参数配置
            print("AGENT NAME: {}".format(agent_name))
            print("\033[1m" + "{}.{}: {}".format(agent_number, agent_round, agent_name) + "\033[0m", flush=True)
            agent = agent_class(agent_config) # 传入配置参数，生成智能体对象
    ...
    def environment_has_changeable_goals(self, env):
        ...
        """确定环境是否使每一个opisode都有不同的目标"""
        return isinstance(env.reset(), dict) # 如果是多个目标就是dict,如果就一个目标就是arry
