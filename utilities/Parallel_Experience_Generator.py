...
class Parallel_Experience_Generator(object):
    """ Plays n episode in parallel using a fixed agent(用一个固定的agent玩n个episode). Only works for PPO or DDPG type agents at the moment, not Q-learning agents"""
    ...
    def play_n_episodes(self, n, exploration_epsilon=None):
        ...
        self.exploration_epsilon = exploration_epsilon # 获取epsilon
        with closing(Pool(processes=n)) as pool: # 指定最大n数量的进程供用户调用
            results = pool.map(self, range(n))
            pool.terminate() # 结束工作进程，不在处理未处理的任务
        states_for_all_episodes = [episode[0] for episode in results] # results大小（7,3,44）
    ...
    def play_1_episode(self, epsilon_exploration):
        ...
        state = self.reset_game() # 重置状态
        done = False
        episode_states = []
        episode_actions = []
        episode_rewards = []
        while not done: # 没结束
            action = self.pick_action(self.policy, state, epsilon_exploration) # 根据一个策略选着一个动作
            next_state, reward, done, _ = self.environment.step(action) # 获得下一个状态，奖励
            if self.hyperparameters["clip_rewards"]: reward = max(min(reward, 1.0), -1.0) # 是否剪裁奖励
            episode_states.append(state) # 保存好状态
    ...
    def reset_game(self):
        ...
        seed = randint(0, sys.maxsize) # 随机种子
